...
class Client:
    ...
    def add_account(self, currency):
        if currency in self.accounts:
            raise Exception(f"Client {self.id_number} have already account in {currency} currency")
    ...
    while True:
        ...
        if choice == "2":
            ...
            try:
                ...
                print(f"Client with ID {client.id_number} opened new account in {currency} currency") 
            ...
            try:    
                ...
                print(f"Client with ID {client.id_number} closed an account in {currency} currency")
            except Exception as e:
                print(f"Error: {e}")
            ...
            try:
                ...
                client.accounts[donate_account].transfer(amount, client.accounts[recipient_account])
                print(f"Amount {amount} was transfered from donate account to recipient account successfully")
            except Exception as e:
                print(f"Error: {e}")
            ...
            try:
                with open(file_name, "w") as file:
                    ...
                    total_balance = sum(account.balance for account in client.accounts.values())
                    for currency, account in client.accounts.items():
                        file.write(f"Account in currency {currency}: balance: {account.balance}\n")
