...
float_numbers=set([float(number) for number in numbers if float(number)!=int(float(number))])

sum_of_aliquot_numbers=sum(int(float(number)) for number in numbers if float(number).is_integer() and int(float(number)) % 5==0)
